...
public class SmartCopy
{
    ...
    private static JObject sourceJsonObject;
    private static HashSet<string> validPublicProperties = new();
    private static Dictionary<string, SerializedPropertyType> expectedPropertyTypes = new();
    private static Dictionary<string, int> objectReferenceValues = new();

    [MenuItem("CONTEXT/Component/Copy Serialized Values")]
    private static void CopySerializedValues(MenuCommand command)
    {
        objectReferenceValues.Clear();
        expectedPropertyTypes.Clear();

        var sourceObject = command.context;
        var serializedSourceObject = new SerializedObject(sourceObject);
        validPublicProperties = GetValidPublicProperties(serializedSourceObject);
        var sourceJson = EditorJsonUtility.ToJson(sourceObject);
        sourceJsonObject = JObject.Parse(sourceJson);
        foreach (var property in validPublicProperties)
        {
            ...
            expectedPropertyTypes.Add(property, serializedProp.propertyType);
            if (serializedProp.propertyType == SerializedPropertyType.ObjectReference)
            {
                objectReferenceValues.Add(property, serializedProp.objectReferenceInstanceIDValue);
            }
        }
    }
    ...
    private static void PasteValuesByPropertyPath(MenuCommand command)
    {
        ...
        var sourceJsonObjectValue = sourceJsonObject.Properties().First().Value;
        ...
        foreach (var property in validPublicProperties)
        {
            ...
            if (serializedProp == null || serializedProp.propertyType == SerializedPropertyType.ObjectReference ||
                serializedProp.propertyType != expectedPropertyTypes[property])
            {
                continue;
            }
        ...
        foreach (var kvp in objectReferenceValues)
        {
    ...
    {
        return sourceJsonObject?.Properties().Any() == true;
    }
